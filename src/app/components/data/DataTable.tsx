...
// Nested interactive elements (compare buttons, links, inputs) live inside
// expandable rows; clicks/keydowns originating from them must not toggle the row.
...
function TableBodyInner<T>({ pagedData, columns, getRowId, isExpandable, expandedRowId, onToggleExpand, renderExpandedRow }: TableBodyProps<T>) {
  return (
    <tbody>
  ...
  return (
    <Fragment>
      <tr
        ...
        className={cn("border-b border-border transition-colors", rowIndex % 2 === 0 ? "bg-bg-card" : "bg-bg-primary", "hover:bg-hover", isExpanded && "bg-accent-light")}
        ...
        {columns.map((col, colIdx) => (
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              {isExpandable && colIdx === 0 && <span className="shrink-0 text-text-secondary">{isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</span>}
              {col.cell(row)}
            </div>
          </td>
        ))}
...
function DataTableInner<T>({ data, columns, getRowId, pageSize = 30, expandedRowId, onToggleExpand, renderExpandedRow }: DataTableProps<T>) {
    ...
    <div className="flex flex-col gap-2">
      {dedupedData.length === 0 ? (
        ...
        <>
          ...
          {dedupedData.length > effectivePageSize && <Pagination page={page} totalPages={totalPages} onChange={goToPage} className="pt-1 self-center" />}
        </>
      )}
    </div>
  );
